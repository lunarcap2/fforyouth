...
<!--#include virtual = "/inc/function/gooroomee_info.asp"-->
...
If IsArray(arrList) Then
	For i = LBound(arrList,2) To UBound(arrList,2)
		...
		If ontactYn = "N" Then ' 해당일시에 기존 개설된 화상 면접 방이 없을 경우 관련 정보 저장
			...
			'1) 면접관용 URL 정보 저장
			Dim hostUrl : hostUrl = ""
			'hostUrl = "https://vc.dial070.co.kr/"&urlCd&"?st="&serviceType_maltalk&"&id="&hostId&"&name="&comname
			...
			'온라인면접여부가 Y인 경우만
			If CDate(interviewDt)=Date() And online_yn = "Y" Then 
				
				Dim set_sdate, set_edate
				set_sdate = getPST(interviewDt, Left(interviewTime, 5))
				set_edate = getPST(interviewDt, Mid(interviewTime, 7, 5))

				'미팅룸 생성 api 호출  참조:/inc/function/gooroomee_info.asp
				json_create_room = create_room(urlCd, "", set_sdate, set_edate)

				'json 파싱
				Set oJSON = New aspJSON
				oJSON.loadJSON(json_create_room)
				result_code = oJSON.data("resultCode")

				If Not result_code = "GRM_200" Then
					Dim str_script : str_script = ""
					str_script = str_script & "<script>"
					str_script = str_script & "	alert('화상면접 생성 오류입니다.');"
					str_script = str_script & "	history.back();"
					str_script = str_script & "</script>"
				...
				Set result_data = oJSON.data("data")
				Set result_data = result_data.item("room")

				sql3 = "SET NOCOUNT ON;"
				sql3 = sql3&"UPDATE 면접배정_면접관URL SET API전송여부='Y', 화상아이디 = '"& result_data.item("roomId") &"' WHERE 채용등록번호='"&jid&"' AND 회사아이디='"&bizid&"' AND 면접일='"&interviewDt&"' AND 면접시간='"&interviewTmCd&"'  " 
				DBCon.Execute(sql3)
			...
			For j = LBound(arrList2,2) To UBound(arrList2,2)
				...
				If Not (Rs.BOF Or Rs.EOF) Then
					...
					sql5 = "insert into arreo_sms (CMP_MSG_ID, CMP_USR_ID, ODR_FG, SMS_GB, USED_CD, MSG_GB, WRT_DTTM, SND_DTTM, SND_PHN_ID, RCV_PHN_ID, CALLBACK, SUBJECT, SND_MSG, EXPIRE_VAL, SMS_ST, RSLT_VAL, RSRVD_ID, RSRVD_WD)" &_
							" values ('" & smsid & "', '00000', '2', '1', '00', 'M', '" & now_time & "', '" & now_time & "', 'daumhr', '" & Replace(Replace(resume_phone, " ", ""),"-","") & "', '"& Replace(Replace(site_callback_phone, " ", ""),"-","") &"', '"&site_name&"', '" & msg & "', 0, '0', 99,'','');"
					DBCon2.Execute(sql5)
				End If
				Rs.Close
				DisconnectDB DBCon2				
									...
									"<tbody>"&_
										...
										"<tr>"&_
											"<td colspan=""2"" style=""padding:20px 20px 0 20px;text-align:right;"">"&_
												...
												"<a href=""https://" & Request.ServerVariables("SERVER_NAME") & "/board/notice_view.asp?seq=" & site_noticeSeq1 & """ target=""_blank"">최소사양 및 화상면접 사용 브라우저 안내</a>&nbsp;"&_
												"<a href=""https://" & Request.ServerVariables("SERVER_NAME") & "/board/notice_view.asp?seq=" & site_noticeSeq2 & """ target=""_blank"">안드로이드 브라우저 설정 안내</a>"&_
											"</td>"&_
										"</tr>"&_
									"</tbody>"&_
								"</table>"&_
				...
				'Set '	= Server.CreateObject("CDO.Message")
