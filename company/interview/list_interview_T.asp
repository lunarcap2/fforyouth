...
<script type="text/javascript">
	...
	function fn_result_submit() {
		...
		for (i=0; i < chk.length; i++)
		{
			...
			console.log(trim(acco_chk) != "8" && trim(acco_textarea) != "");
			console.log(trim(acco_chk) == "8");
			console.log(trim(acco_chk));
			console.log((trim(acco_chk) != "8" && trim(acco_textarea) != "") || (trim(acco_chk) == "8"));
	...
	function fn_msg_resend(jid, applyid, name, flag, confid){
		if (confid == "") {
			...
			var strMent = "님에게 면접일정 안내 문자/메일을 발송하시겠습니끼?";
			if (flag != ""){
				strMent = "님에게 면접일정 안내 문자/메일을 재발송하시겠습니끼?";	
			}

			if(confirm(name+strMent)) {
				var url = "ontact_msg_resend_copy.asp?jid="+jid+"&applyid="+applyid;
				location.href = url;	
			}
		}
	}
...
%>	
					...
					<div class="slide_area">
						...
						For i=0 To UBound(arrRsTimeToCnt, 2)
							...
							sql4 = sql4 & " WHERE 채용등록번호='"&jid&"' AND 회사아이디='"&comid&"' AND 채용등록번호='"&jid&"' AND 면접일='"&interview_day&"' AND 면접시간='"&arrRsTimeToCnt(2, i)&"' "
